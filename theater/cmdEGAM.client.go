...
func (tM *TheaterManager) EGAM(event GameSpy.EventClientFESLCommand) {
	...
	// Get client data
	stmt, err := tM.db.Prepare("SELECT web_id, nickname FROM west_soldiers WHERE pid = ?")
	defer stmt.Close()
	if err != nil {
		log.Errorln(err)
		return
	...
	// Get statas data
	statsStmt, err := tM.db.Prepare("SELECT c_kit, c_team, elo, level FROM west_heroes_stats WHERE pid = ?")
	defer stmt.Close()
	if err != nil {
		log.Errorln(err)
		return
