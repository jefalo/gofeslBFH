...
func (fM *FeslManager) NuLookupUserInfo(event GameSpy.EventClientTLSCommand) {
	...
	stmt, err := fM.db.Prepare("SELECT nickname, web_id, pid FROM west_soldiers WHERE nickname IN (?" + strings.Repeat(",?", len(userNames)-1) + ")")
	defer stmt.Close()
	if err != nil {
		log.Errorln(err)
		return
...
func (fM *FeslManager) NuGetPersonas(event GameSpy.EventClientTLSCommand) {
	...
	if event.Client.RedisState.Get("clientType") == "server" {
		...
		// Server login
		stmt, err := fM.db.Prepare("SELECT name, id FROM west_heroes_servers WHERE id = ?")
		log.Noteln(stmt)
		defer stmt.Close()
		if err != nil {
			return
	...
	stmt, err := fM.db.Prepare("SELECT nickname, pid FROM west_soldiers WHERE web_id = ?")
	log.Noteln(stmt)
	defer stmt.Close()
	if err != nil {
		return
...
func (fM *FeslManager) GetStatsForOwners(event GameSpy.EventClientTLSCommand) {
	...
	stmt, err := fM.db.Prepare("SELECT " + query + "pid FROM west_heroes_stats WHERE pid IN (?" + strings.Repeat(",?", len(pids)-1) + ")")
	defer stmt.Close()
	if err != nil {
		log.Errorln(err)
		return
